...
- Resolution paths: (1) accepted explanation → words-learned increments; overlay closes; Next Page unlocks. (2) rejected with retries left → live why-reason + live hint from the grade call; stay in overlay; try again. (3) grading unavailable → burn attempt; pre-written fallback hint; stay in overlay unless retry limit. (4) retry limit reached (wrong grades or grading failures) → reveal the word’s meaning in kid-friendly language; overlay closes; Next Page unlocks (no words-learned increment).
